...
namespace CERP.Web.Areas.HR.Setup.OrganizationalManagement.PayrollStructure.Pages.PayFrequencies
{
    public class ListModel : CERPPageModel
    {
        ...
        public PS_PayFrequencyAppService PS_PayFrequencyAppService { get; set; }

        public IAuditingManager AuditingManager { get; set; }
        public IRepository<CustomEntityChange> CustomEntityChangesRepo { get; set; }

        public ListModel(IJsonSerializer jsonSerializer, IRepository<DictionaryValue, Guid> dictionaryValuesRepo, IWebHostEnvironment webHostEnvironment, IAuditLogRepository auditLogsRepo, PS_PayFrequencyAppService oS_PayFrequencyAppService)
        {
            JsonSerializer = jsonSerializer;
            DictionaryValuesRepo = dictionaryValuesRepo;
            this.webHostEnvironment = webHostEnvironment;
            AuditLogsRepo = auditLogsRepo;
            PS_PayFrequencyAppService = oS_PayFrequencyAppService;
        }
        ...
        public async Task<IActionResult> OnPostPayFrequency()
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    PS_PayFrequency_Dto payFrequency_Dto = JsonSerializer.Deserialize<PS_PayFrequency_Dto>(FormData["info"]);

                    bool IsEditing = payFrequency_Dto.Id > 0;
                    if (IsEditing)
                    {
                        PS_PayFrequency curPayFrequency = await PS_PayFrequencyAppService.Repository.GetAsync(payFrequency_Dto.Id);

                        if (AuditingManager.Current != null)
                        {
                            EntityChangeInfo entityChangeInfo = new EntityChangeInfo();

                            entityChangeInfo.EntityId = payFrequency_Dto.Id.ToString();
                            entityChangeInfo.EntityTenantId = payFrequency_Dto.TenantId;
                            entityChangeInfo.ChangeTime = DateTime.Now;
                            entityChangeInfo.ChangeType = EntityChangeType.Updated;
                            entityChangeInfo.EntityTypeFullName = typeof(PS_PayFrequency).FullName;
                            ...
                            var auditProps = typeof(PS_PayFrequency).GetProperties().Where(x => Attribute.IsDefined(x, typeof(CustomAuditedAttribute))).ToList();

                            PS_PayFrequency mappedInput = ObjectMapper.Map<PS_PayFrequency_Dto, PS_PayFrequency>(payFrequency_Dto);
                            foreach (var prop in auditProps)
                            {
                                EntityPropertyChangeInfo propertyChange = new EntityPropertyChangeInfo();
                                object origVal = prop.GetValue(curPayFrequency);
                                ...
                                if (prop.Name.EndsWith("Id"))
                                {
                                    try
                                    {
                                        ...
                                        var valueProp = typeof(PS_PayFrequency).GetProperty(valuePropName);

                                        DictionaryValue _origValObj = (DictionaryValue)valueProp.GetValue(payFrequency_Dto);
                            ...
                            //if (department_Dto.ExtraProperties != curPayFrequency.ExtraProperties)
                            //{
                        ...
                        curPayFrequency.Name = payFrequency_Dto.Name;
                        curPayFrequency.NameLocalized = payFrequency_Dto.NameLocalized;
                        curPayFrequency.Code = payFrequency_Dto.Code;
                        curPayFrequency.Description = payFrequency_Dto.Description;
                        curPayFrequency.AnnualizationFactor = payFrequency_Dto.AnnualizationFactor;

                        PS_PayFrequency_Dto updated = ObjectMapper.Map<PS_PayFrequency, PS_PayFrequency_Dto>(await PS_PayFrequencyAppService.Repository.UpdateAsync(curPayFrequency));

                        return StatusCode(200, updated);
                    }
                    else
                    {
                        payFrequency_Dto.Id = 0;

                        PS_PayFrequency_Dto added = await PS_PayFrequencyAppService.CreateAsync(payFrequency_Dto);
                        added = await PS_PayFrequencyAppService.GetAsync(added.Id);
        ...
        public async Task<IActionResult> OnDeletePayFrequency()
        {
            List<PS_PayFrequency_Dto> entitites = JsonSerializer.Deserialize<List<PS_PayFrequency_Dto>>(Request.Form["payFrequencys"]);
            try
            {
                for (int i = 0; i < entitites.Count; i++)
                {
                    PS_PayFrequency_Dto entity = entitites[i];
                    //await PayFrequenciesAppService.Repository.DeleteAsync(leaveRequest.);
                    await PS_PayFrequencyAppService.Repository.DeleteAsync(entity.Id);
        ...
        public async Task<JsonResult> OnGetDataAuditTrail()
        {
            ...
            List<PS_PayFrequency_Dto> Entities = await PS_PayFrequencyAppService.GetAllPayFrequenciesAsync();
            ...
            for (int i = 0; i < departmentLogs.Count; i++)
            {
                ...
                for (int j = 0; j < entityChanges.Count; j++)
                {
                    ...
                    PS_PayFrequency_Dto department = Entities.First(x => x.Id.ToString() == entityChange.EntityId);
